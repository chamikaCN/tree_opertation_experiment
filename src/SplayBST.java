...
public class SplayBST<Key extends Comparable<Key>, Value> {
    ...
    public static void main(String[] args) {
        SplayBST<Long, Integer> spt = null;
        Helper helper = new Helper();
        long timer = 0;
        ArrayList<Long> keyvalues;
        ArrayList<Long> searchValues;
        ArrayList<Long> deleteValues;
        ArrayList<Long> executionTimes = new ArrayList<>();

        for (int j = 1; j < 4; j++) {
            spt = new SplayBST<>();
            keyvalues = helper.readFile("insert", 1, j);
            timer = System.nanoTime();
            for (long key : keyvalues) {
                spt.put(key, 1);
            }
            executionTimes.add((System.nanoTime() - timer) / 1000);
            searchValues = helper.readFile("search", 1, j);
            timer = System.nanoTime();
            for (long searchKey : searchValues) {
                spt.contains(searchKey);
            }
            executionTimes.add((System.nanoTime() - timer) / 1000);
            deleteValues = helper.readFile("delete", 1, j);
            for (long deleteKey : deleteValues) {
                spt.put(deleteKey, 1);
            }
            timer = System.nanoTime();
            for (long deleteKey : deleteValues) {
                spt.remove(deleteKey);
            }
            executionTimes.add((System.nanoTime() - timer) / 1000);
        }
        ...
        try {
            BufferedWriter writer = new BufferedWriter(new FileWriter("DataSet1SPT.txt", true));
